...
首先介绍组成网页的框架中一般有哪些文件类型。

- HTML（超文本标记语言）：构建网页内容的基础，它通过标签来描述**网页的结构布局**。
- CSS（层叠样式表）：用于控制网页的**外观和样式**，如字体、颜色、布局等，使网页变得美观和易于阅读。
- JavaScript：网页的编程语言，主要用于网页中的**交互性和动态效果**。
...
* `_pages`文件夹用于定义网页的内容，使用markdown文件。
...
官方主题提供了多种页面布局（如 *splash*、*single* 等），这些布局文件均位于 `_layouts` 目录中。为了实现一个更加个性化的主页页面，例如自定义横幅（Hero Section）并在其中加入打字机动画效果，我们需要编写一个新的首页布局文件。

首先，新建一个用于控制主页结构的布局文件 `homepage.html`。该文件将继承官方提供的 `default.html` 布局，因为 `default.html` 中已经定义了整站通用的头部导航栏、页脚等全局结构。

`homepage.html` 的完整代码如下：
...
该文件主要由两个部分组成。第一部分是用于显示首页横幅的 `<div>` 元素，它的样式由 `home__image` 类控制。此外，该元素还可能带有一个修饰类 `animate-reveal`，两者组合为 `.home__image.animate-reveal`，用于在横幅上启用展开动画效果。横幅的宽度、高度、背景颜色等属性则通过 `style` 行内样式进行控制。

在横幅内部，还包含一个用于承载文本的 `<div>` 元素，并通过类名 `content` 来定义文本的布局方式，例如水平垂直居中、定位方式等。对应的样式如下：
...
其中 `&.animate-reveal` 表示对 `.home__image` 这个类的修饰写法。**只有当元素同时具有 `home__image` 与 `animate-reveal` 这两个类时**，该样式规则才会生效；如果元素只有 `home__image` 而不包含 `animate-reveal`，则不会应用这部分动画样式。

而嵌套在 `.home__image` 内的 `.content` 则表示**无论父元素是 `.home__image` 还是 `.home__image.animate-reveal`，只要某个子元素的类名为 `content`，都将应用这一样式**。因此 `.content` 仅与“父元素属于 `.home__image`”相关，与是否带有 `animate-reveal` 无关。
...
function typewriter(el, text, options = {}) {
    ...
    const loop = options.loop ?? false; //是否让动画循环播放
    const pauseAfter = options.pauseAfter ?? 1200; // 循环播放时全文显示后停留时间
    let i = 0;
    el.textContent = "";
    function step() {
        if (i < text.length) {
            ...
            el.classList.remove("caret-visible"); //动画结束后消除光标
...
该文件主要由两部分组成：**打字机动画函数的定义** 和 **页面加载后的事件注册逻辑**。

第一部分负责定义实现文本逐字显示效果的函数 `typewriter`。该函数会根据指定的时间间隔逐个输出字符，并在必要时控制光标的显示与隐藏，从而形成打字机动画。

第二部分通过注册事件监听器来控制动画启动时机。例如，`DOMContentLoaded` 事件会在 **HTML 完成解析、DOM 结构构建完成** 时触发，此时可以安全访问页面元素并初始化动画。
 其中使用的 `document.querySelector` 是 JavaScript 的 DOM 查询方法，用于获取文档中第一个匹配指定 CSS 选择器的元素。

代码中还出现了箭头函数的写法，例如 `const handler = (e) => { ... }`，其中 `handler` 是函数名，而 `e` 是事件对象，包含当前事件的相关信息。此外，`setTimeout(function, delay, ...)` 是定时器函数，用于在指定的 `delay` 毫秒后执行对应的函数。在本文件中，它用于控制动画的延迟执行，如等待父元素动画结束或设置文字逐字输出的速度

在完成以上文件设置后，即可在主页看到完整的动画显示。
...
由于GitHub Pages默认使用Jekyll + kramdown来渲染Markdown文件，导致无法自动渲染`$...$`或者是`$$...$$`里的数学公式。因此这里引入MathJax来解决。

**1.在_includes文件夹中添加*mathjax_support.html*文件，并输入以下代码**
...
**2.在_layouts/default.html文件的head部分中加入{% include mathjax_support.html %}**
